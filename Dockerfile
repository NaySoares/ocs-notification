...
RUN yum install ${YUM_FLAGS} python3-pip && \
    pip install ${YUM_FLAGS} requests discord discord-webhook

RUN wget https://gl.idc.ufpa.br/ocs_inventory-ufpa/2.8/-/archive/v1.0/2.8-v1.0.tar.gz && \
    tar -xzf 2.8-v1.0.tar.gz;

RUN mv /tmp/2.8-v1.0 /tmp/OCSNG_UNIX_SERVER_2.8;
